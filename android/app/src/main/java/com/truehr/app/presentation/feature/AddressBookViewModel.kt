...
class AddressBookViewModel @Inject constructor(
  ...
  val list = MutableStateFlow(UiState<List<DirectoryEntry>>())
  fun load() = viewModelScope.launch {
    list.update { it.copy(loading = true, error = null) }
    try { list.value = UiState(data = repo.directory()) }
    catch (e: Exception) { list.value = UiState(error = e.apiMessage("Failed to load address book")) }
  }
}
