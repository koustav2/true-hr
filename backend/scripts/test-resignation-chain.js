...
async function seed() {
  ...
  const hr = await mk('Hh'); const it = await mk('It'); const oa = await mk('Oa'); const fin = await mk('Fi');
  ...
  for (const [role, who] of [['IT_INFRA', it], ['OFFICE_ADMIN', oa], ['FINANCE', fin], ['HR', hr]]) {
    await query(`UPDATE approval_flow_stages SET default_approver_employee_id=$2 WHERE flow_id=$1 AND role_key=$3`, [flowId, who, role]);
  }
  return { emp, rm, fm, it, oa, fin, hr };
}
...
async function main() {
  ...
  check('stage roles match GreenHR demo', r.data.chain.map((s) => s.roleKey).join(',') ===
    'REPORTING_MANAGER,FUNCTIONAL_HEAD,IT_INFRA,OFFICE_ADMIN,FINANCE,HR');
  ...
  for (const [who, label] of [[S.rm, 'RM'], [S.fm, 'FH'], [S.it, 'IT'], [S.oa, 'OA'], [S.fin, 'FIN']]) {
